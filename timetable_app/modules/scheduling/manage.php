...
if (isset($_POST['add_entry'])) {
    ...
    // Conflict Check
    $stmt = $pdo->prepare("SELECT * FROM timetable WHERE slot_id = ? AND (room_id = ? OR teacher_id = ? OR (class_id = ? AND group_name = ?)) AND semester_id = ?");
    $stmt->execute([$slot_id, $room_id, $teacher_id, $class_id, $group_name, $semester_id]);
    if ($stmt->fetch()) {
        ...
        if ($class_size > $room_capacity) {
            ...
            $stmt = $pdo->prepare("INSERT INTO timetable (class_id, course_id, teacher_id, room_id, slot_id, semester_id, group_name, date_passage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$class_id, $course_id, $teacher_id, $room_id, $slot_id, $semester_id, $group_name, $date_passage]);
...
$timetable = $pdo->query("SELECT t.*, c.name as class_name, co.code as course_code, te.name as teacher_name, r.name as room_name, s.day, s.start_time, s.end_time FROM timetable t JOIN classes c ON t.class_id = c.id JOIN courses co ON t.course_id = co.id JOIN teachers te ON t.teacher_id = te.id JOIN rooms r ON t.room_id = r.id JOIN slots s ON t.slot_id = s.id")->fetchAll();
        ...
        <tbody>
            <?php foreach ($timetable as $entry): ?>
                <tr>
                    ...
                    <td><?php echo $entry['day']; ?> <?php echo $entry['start_time']; ?></td>
                    <td>
